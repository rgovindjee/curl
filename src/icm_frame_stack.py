...
class IcmFrameStack(VecFrameStack):
    ...
    def __init__(self,
        ...
        self.n_actions = 4
        self.icm = ICMModel(output_size=self.n_actions,
                            device=device, obs_space=venv.observation_space)
        ...
        self.logger = configure_logger(
            verbose=1, tensorboard_log=log_path, tb_log_name="ICM")
    ...
    def step_wait(self):
        ...
        observations, infos = self.stacked_obs.update(
        ...
        intrinsic_reward = self.eta * \
            F.mse_loss(real_next_state_feature,
                       pred_next_state_feature, reduction='none').mean(-1)

        # Caclulate the forward and inverse model losses
        ce = nn.CrossEntropyLoss()
        forward_mse = nn.MSELoss()
        # Use this to train the inverse model
        y_action = self.icm.actions_to_tensor(self.action)
        inverse_loss = ce(
        ...
        if self.iterations % 60 == 0:
            self.logger.record("train/forward_loss_icm",
                               np.mean(self.to_numpy(forward_loss)))
            self.logger.record("train/inverse_loss_icm",
                               np.mean(self.to_numpy(inverse_loss)))
            self.logger.record("train/curiosity_reward",
                               np.mean(self.to_numpy(intrinsic_reward)))
            self.logger.record(
...
class ICMModel(nn.Module):
    ...
    def observations_to_tensor(self, obs):
        ...
        t_unsqueezed = [torch.unsqueeze(t, dim=0) for t in tensors]
    ...
    def forward(self, inputs):
        ...
        state = preprocess_obs(
            self.observations_to_tensor(state), self.obs_space)
        next_state = preprocess_obs(
            self.observations_to_tensor(next_state), self.obs_space)
        ...
        pred_next_state_feature_orig = self.forward_net_1(
